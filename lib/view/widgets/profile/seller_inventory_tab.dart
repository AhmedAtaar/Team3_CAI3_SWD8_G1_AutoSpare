...
class _SellerInventoryTabState extends State<SellerInventoryTab> {
  ...
  void initState() {
    super.initState();
    ordersRepo.watchSellerOrders(widget.sellerId).listen((orders) {
  ...
  Future<void> _openRestockDialog(CatalogProduct p) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: Text('زيادة مخزون — ${p.title}', textDirection: TextDirection.rtl),
        content: Form(
          key: formKey,
          child: TextFormField(
            controller: qtyCtrl,
    ...
    if (ok == true) {
      ...
      final done = Catalog().restock(p.id, delta);
      if (!mounted) return;
      if (done) {
        setState(() {}); // نحدّث الـ UI
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('تمت إضافة $delta إلى مخزون "${p.title}"', textDirection: TextDirection.rtl)),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('تعذر تعديل المخزون'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final products = Catalog()
        .all()
        .where((p) => p.seller == widget.sellerId)
        .toList()
    ...
    if (products.isEmpty) {
      return Center(
        child: Padding(
          ...
          child: Text('لا توجد منتجات مقبولة لهذا البائع في الكتالوج'),
        ),
      );
    }
    ...
    Widget _header() => Container(
      ...
      child: Wrap(
        ...
        runSpacing: 8,
        children: [
          Chip(label: Text('إجمالي المخزون: $totalStock'), avatar: const Icon(Icons.inventory_2_outlined)),
          Chip(label: Text('إجمالي المبيعات: $totalSold'),  avatar: const Icon(Icons.shopping_bag_outlined)),
          Chip(label: Text('الإيراد الكلي: ${totalRevenue.toStringAsFixed(2)}'), avatar: const Icon(Icons.payments_outlined)),
        ],
      ),
    );
                            ...
                            Wrap(
                              ...
                              children: [
                                Chip(
                                  ...
                                  backgroundColor: low ? cs.errorContainer : cs.surfaceContainer,
                                  labelStyle: TextStyle(color: low ? cs.onErrorContainer : cs.onSurface),
                                ),
                                Chip(
                                ...
                                Chip(
                                  ...
                                  label: Text('الإيراد: ${revenue.toStringAsFixed(2)}'),
                                ),
                                Chip(
                                  avatar: const Icon(Icons.tag_outlined, size: 18),
                                  label: Text('السعر: ${p.price.toStringAsFixed(2)}'),
                                ),
                              ],
                            ),
                            const SizedBox(height: 4),
                            Text('سنوات الملاءمة: ${p.years.join(', ')}'),
                          ],
                        ),
                      ),
                    ],
